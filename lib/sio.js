  ...
  connect() {
    this.io.on('connection', (socket) => {
      ...
      socket.on('run code', (data) => {
        ...
        let currentFileName;
        createFile(data.langauge, data.text)
          .then((passedFileName) => {
            currentFileName = passedFileName;
            return runCode(data.langauge, passedFileName, data.text);
          })
          .then((response) => {
            this.io.in(data.room).emit('ran code', response);
